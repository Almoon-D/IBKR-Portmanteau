...
Copyright (C) 2026 Almoon.D - Licensed under the GNU GPLv3
"""
...
IBKR_TICKLE_SECS   = 45
CACHE_DB_PATH      = os.environ.get("FINANCIAL_MCP_CACHE_DB", "cache.db")
...
# Cache retention configurations
...
class AsyncCacheManager:
    ...
    async def init(self) -> None:
        if aiosqlite is None:
            log.warning("aiosqlite not installed. Cache will be disabled.")
            return
        self._db = await aiosqlite.connect(self.db_path)
        await self.db.execute("CREATE TABLE IF NOT EXISTS mcp_cache (key TEXT PRIMARY KEY, value TEXT NOT NULL, expires REAL NOT NULL)")
        await self.db.commit()
        log.info("Asynchronous AIOSQLITE cache active.")

    async def get(self, key: str) -> Optional[Any]:
        if self._db is None: return None
        async with self.db.execute("SELECT value, expires FROM mcp_cache WHERE key = ?", (key,)) as cursor:
            row = await cursor.fetchone()
            if not row: return None
            value, expires = row
            if time.time() > expires:
                await self.db.execute("DELETE FROM mcp_cache WHERE key = ?", (key,))
                await self.db.commit()
                return None
    ...
    async def set(self, key: str, value: Any, ttl_secs: int) -> None:
        ...
        await self.db.execute("INSERT OR REPLACE INTO mcp_cache (key, value, expires) VALUES (?, ?, ?)", (key, json.dumps(value), expires))
        await self.db.commit()
...
class IBKRGateway:
    ...
    async def start(self) -> None:
        await self.check_immediate_status()
    ...
    async def check_immediate_status(self) -> None:
        try:
...
class MarketFacade:
    ...
    async def get_spot_price(self, ticker: str, asset_class: str) -> dict[str, Any]:
        ...
        if asset_class == "CRYPTO":
            async with self.semaphore:
                try:
                    ...
                    results = await asyncio.gather(spot_task, fund_task, return_exceptions=True)
                    spot_resp = results[0]
                    fund_resp = results[1]
                    
                    if isinstance(spot_resp, Exception) or not isinstance(spot_resp, httpx.Response) or spot_resp.status_code != 200:
                        return {"error": "Asset not found on Binance Spot", "source": "ERROR"}
                    spot_data = spot_resp.json()
                    funding_rate = None
                    if not isinstance(fund_resp, Exception) and isinstance(fund_resp, httpx.Response) and fund_resp.status_code == 200:
        ...
        async with self.semaphore:
            try:
                ...
                def fetch_yf_info() -> Any:
                    return yf.Ticker(ticker).fast_info
                info = await loop.run_in_executor(None, fetch_yf_info)
    ...
    async def get_historical_ohlcv(self, ticker: str, interval: str, period: str, limit: int) -> dict[str, Any]:
        ...
        if cached: return cached
        async with self.semaphore:
            if yf is None: return {"error": "yfinance not installed. Fallback unavailable."}
            try:
                loop = asyncio.get_running_loop()
                def fetch_yf_hist() -> Any:
                    return yf.Ticker(ticker).history(period=period, interval=interval)
                df = await loop.run_in_executor(None, fetch_yf_hist)
                if df is None or df.empty: return {"error": "Historical data unavailable"}
                bars = []
    ...
    async def get_fx_rate(self, base: str, quote: str) -> dict[str, Any]:
        ...
        async with self.semaphore:
            ...
            try:
                loop = asyncio.get_running_loop()
                def fetch_yf_fx() -> Any:
                    return yf.Ticker(pair).fast_info
                info = await loop.run_in_executor(None, fetch_yf_fx)
    ...
    async def get_options_chain_with_greeks(self, ticker: str, max_dte: int, moneyness: float, rate_pct: float) -> dict[str, Any]:
        ...
        if "error" in spot_dict:
            return {"error": f"Underlying price lookup failed: {spot_dict['error']}"}
            
        spot_price = _safe_float(spot_dict.get("last"))
        if spot_price <= 0:
            return {"error": "Cannot calculate options chain without a valid spot price."}
        if self.ibkr.healthy:
            try:
                async with self.ibkr_semaphore:
                    conid = await self.ibkr.resolve_conid(ticker)
        ...
        async with self.semaphore:
            ...
            try:
                ...
                def fetch_yf_options() -> Any:
                    return yf_ticker.options
                expirations = await loop.run_in_executor(None, fetch_yf_options)
                if not expirations: return {"error": "No options expirations found."}
                ...
                for exp, chain in zip(target_expirations, chains):
                    if isinstance(chain, Exception) or chain is None: continue
                    # chain is expected to have 'calls' and 'puts' attributes (DataFrames)
                    calls = getattr(chain, "calls", None)
                    puts = getattr(chain, "puts", None)
                    if calls is None or puts is None: continue
...
if FastMCP is not None:
    ...
    class MockMCP:
        ...
        def run(self, *args, **kwargs):
            log.critical("Cannot run: FastMCP library missing.")
    mcp = MockMCP()  # type: ignore
    log.error("FastMCP library not found. Server will not be functional.")
...
async def search_instrument(query: str) -> str:
    ...
    if _market is None: return json.dumps({"error": "Market facade not initialized."})
...
async def get_portfolio_summary() -> str:
    ...
    if _portfolio is None: return json.dumps({"error": "Portfolio engine not initialized."})
...
async def get_global_macro_scanner(indicator_id: str, region: str) -> str:
    ...
    if _macro is None: return json.dumps({"error": "Macro provider not initialized."})
...
if __name__ == "__main__":
    ...
    if mcp:
        ...
        log.critical("Server instance not available. Check dependencies (FastMCP).")